...
public class AstroLib {
	...
	public static double getRefraction(final Location loc, double sunAlt)
	{
	    ...
	    part2 = 1 / Math.tan(Utils.DEG_TO_RAD(sunAlt + (7.31/(sunAlt + 4.4)))) + 0.0013515;
	...
	public static void computeAstroDay(double JD, AstroDay astroday)
	{
	    ...
	    DEC = Math.asin( Math.sin(Utils.DEG_TO_RAD(Gg)) * Math.cos(Utils.DEG_TO_RAD(E)) + 
	                            Math.cos(Utils.DEG_TO_RAD(Gg)) * Math.sin(Utils.DEG_TO_RAD(E)) * 
	...
	public static void computeTopAstro(final Location loc, final Astro astro, Astro topAstro)
	{
	    ...
	    for (i=0; i<3; i++)
	    {
	        ...
	        tRA0 = (((-tCos) * Math.sin(Utils.DEG_TO_RAD(SP)) * Math.sin(Utils.DEG_TO_RAD(lHour))) 
	                / (Math.cos(astro.getDec()[i]) - tCos * Math.sin(Utils.DEG_TO_RAD(SP)) * Math.cos(Utils.DEG_TO_RAD(lHour))));


	        tRA = astro.getRa()[i] +  Utils.RAD_TO_DEG(tRA0);


	        tDEC = Utils.RAD_TO_DEG(Math.atan2((Math.sin(astro.getDec()[i]) - tSin * Math.sin(Utils.DEG_TO_RAD(SP))) * Math.cos(tRA0), 
	                                Math.cos(astro.getDec()[i]) - tCos * Math.sin(Utils.DEG_TO_RAD(SP)) * 
